...
class UniquenessOfPostRule implements Rule
{
	...
	public function passes($attribute, $value)
	{
		...
		// Check if this user hasn't yet posted a listing with this title in the same country
		$posts->where('title', 'LIKE', $value);
		
		// Add country code to the uniqueness check to allow same title in different countries
		$countryCode = request()->input('country_code') ?: config('country.code');
		$posts->where('country_code', $countryCode);
		
		// Don't pass, If a listing with the same title found for the user in the same country
		if ($posts->count() > 0) {
