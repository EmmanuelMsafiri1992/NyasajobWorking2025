...
use App\Notifications\JobSeekerPostNotification;
...
class PostObserver
{
	...
	public function created(Post $post)
	{
		...
		// Send Job Seeker Notification Email
		if (config('settings.mail.job_seeker_notification') == '1') {
			try {
				// Get all verified job seekers in the same country
				$jobSeekers = User::whereDoesntHave('roles', function ($query) {
					$query->whereIn('name', Permission::getStaffPermissions());
				})
				...
				if ($jobSeekers->count() > 0) {
					Notification::send($jobSeekers, new JobSeekerPostNotification($post));
				}
			} catch (\Throwable $e) {
			}
		}
